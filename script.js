...
function renderAuthLoggedOut(){
 ...
 <input id="password" type="password" autocomplete="current-password" placeholder="6 caractères minimum">
 ...
 <input id="phoneLogin" type="tel" autocomplete="tel" placeholder="+221 77 000 00 00">

 <button class="secondary full" id="sendPhoneLogin">
   📲 Recevoir le code SMS
 </button>

 <div id="phoneLoginOtpArea" class="hidden">
   <label>Code reçu par SMS</label>

   <input id="phoneLoginOtp"
     inputmode="numeric"
     autocomplete="one-time-code"
     maxlength="6"
     placeholder="Code à 6 chiffres">

   <button class="primary full" id="verifyPhoneLogin">
     ✅ Vérifier le code
   </button>
 </div>
 ...
 $("sendPhoneLogin").onclick=sendPhoneLoginOtp;
 $("verifyPhoneLogin").onclick=verifyPhoneLoginOtp;
}
...
async function renderAccount(){
 ...
 const phone=p?.phone||session.user.phone||"";
 ...
 <label>Prénom</label>
 <input id="profileFirst" value="${esc(first)}">

 <label>Nom</label>
 <input id="profileLast" value="${esc(last)}">

 <label>Téléphone</label>
 <input id="profilePhone" type="tel" value="${esc(phone)}">
...
async function auth(mode){
 if(!sb){
   return $("authInfo").textContent="Connexion indisponible.";
 }
 ...
 if(!email||!password){
   return $("authInfo").textContent="Remplis email et mot de passe.";
 }
...
async function oauth(provider){
 if(!sb){
   return toast("Supabase indisponible");
 }
...
async function sendPhoneLoginOtp(){
 ...
 const phone=normalizePhone($("phoneLogin").value);

 if(!phone){
   $("authInfo").textContent="Entre ton numéro de téléphone.";
   return;
 }
 ...
 $("phoneLoginOtpArea").classList.remove("hidden");
...
async function verifyPhoneLoginOtp(){
 ...
 const phone=normalizePhone($("phoneLogin").value);
 const token=$("phoneLoginOtp").value.trim();
...
async function saveProfile(){
 ...
 $("accountInfo").textContent=
   r.error?r.error.message:"Profil enregistré ✓";
}
...
async function loadMyOrders(){
 ...
 $("ordersList").innerHTML=r.error
   ?`<p class="status">${esc(r.error.message)}</p>`
   :r.data?.length
     ?r.data.map(o=>`<div class="order glass">
        <strong>${money(o.total)}</strong>
        <small>${new Date(o.created_at).toLocaleString("fr-FR")} · ${esc(o.status)}</small>
       </div>`).join("")
     :`<p class="auth-note">Aucune commande.</p>`;
}
...
async function submitOrder(method){
 ...
 if(!first||!last||!phone){
  return toast("Remplis tous les champs");
 }

 if(!phoneVerified){
  return toast("Vérifie ton numéro avant la commande");
 }
...
async function sendOtp(){
 if(!sb||!session){
   return toast("Connecte-toi d'abord");
 }

 const phone=normalizePhone($("orderPhone").value);

 if(!phone){
   return toast("Entre ton numéro");
 }
 ...
 if(r.error){
   return toast("La vérification SMS n'est pas configurée dans Supabase.");
 }
...
async function loadAdmin(){
 if(!session){
  $("adminProducts").innerHTML=
    `<p class="auth-note">Connecte-toi avec le compte administrateur.</p>`;
  return;
 }
...
async function loadAdminProducts(){
 ...
 $("adminProducts").innerHTML=r.data?.length
   ?r.data.map(p=>`<div class="admin-row glass">
      <img src="${esc(p.image||"logo-mhd-shop.png")}" alt="">
      <div class="grow">
        <b>${esc(p.name)}</b>
        <small>
          ${esc(p.category||"")} · ${money(p.price)} ·
          ${p.available===false?"Indisponible":"Disponible"}
        </small>
      </div>
      <div class="admin-actions">
        <button class="danger" data-del="${esc(p.id)}">
          Supprimer
        </button>
      </div>
    </div>`).join("")
   :`<p class="auth-note">Aucun produit.</p>`;
}
...
async function loadAdminOrders(){
 ...
 $("adminOrders").innerHTML=orders.length
   ?orders.map(o=>`<div class="order glass">
      ...
      ${o.status!=="completed"
        ?`<button class="secondary full" data-complete="${esc(o.id)}">
            Marquer terminée
          </button>`
        :""}
    </div>`).join("")
...
async function saveProduct(){
 ...
 if(!name||!cat||!price){
   return toast("Remplis les champs du produit");
 }
...
document.addEventListener("click",async e=>{
 ...
 if(add){
  const p=products.find(
    x=>String(x.id)===String(add.dataset.add)
  );
 ...
 if(complete){
  const r=await sb.from("orders")
    ...
    .eq("id",complete.dataset.complete);
 ...
 if(ai){
  const answers={
   produits:"Je peux t'aider à trouver un produit dans la boutique. 🛍️",
   commande:"Pour commander, ajoute un produit au panier puis passe à la commande. 📦",
   support:"Support MHD SHOP : +221 78 748 81 99. 💬"
  };

  addAi(answers[ai.dataset.ai]);
  return;
 }
...
function addAi(text){
 $("aiMessages").insertAdjacentHTML(
   "beforeend",
   `<div class="ai-msg bot">${esc(text)}</div>`
 );
...
$("orderOnSite").onclick=()=>{
 ...
 $("orderOnSite").classList.add("selected");
...
$("orderOnWhatsApp").onclick=()=>{
 ...
 $("orderOnWhatsApp").classList.add("selected");
...
$("aiSend").onclick=()=>{
 ...
 $("aiMessages").insertAdjacentHTML(
   "beforeend",
   `<div class="ai-msg user">${esc(t)}</div>`
 );
